...
def compute_sentence_transformer_embeddings(texts):
...
def build_item_profiles_sentence_transformer(business_df, reviews_df):
    ...
    embeddings = compute_sentence_transformer_embeddings(merged_df['review_text'].tolist())
    return {row['business_id']: embeddings[idx] for idx, row in merged_df.iterrows()}


def recommend_similar_businesses_sentence_transformer(business_id, item_profiles, top_n=5):
    """Recommend similar businesses using Sentence Transformer embeddings."""
...
if __name__ == "__main__":
    ...
    profiles = build_item_profiles_sentence_transformer(business_df, reviews_df)
    sample_business_id = business_df['business_id'].iloc[0]
    recommendations = recommend_similar_businesses_sentence_transformer(sample_business_id, profiles, top_n=5)
