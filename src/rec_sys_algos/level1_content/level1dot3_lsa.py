...
def compute_lsa_embeddings(texts, n_components=100):
...
def build_item_profiles_lsa(business_df, reviews_df):
    ...
    embeddings = compute_lsa_embeddings(merged_df['review_text'].tolist())
    return {row['business_id']: embeddings[idx] for idx, row in merged_df.iterrows()}


def recommend_similar_businesses_lsa(business_id, item_profiles, top_n=5):
    """Recommend similar businesses using LSA embeddings."""
...
if __name__ == "__main__":
    ...
    profiles = build_item_profiles_lsa(business_df, reviews_df)
    sample_business_id = business_df['business_id'].iloc[0]
    recommendations = recommend_similar_businesses_lsa(sample_business_id, profiles, top_n=5)
